...
void Ping(char* cHost, unsigned int Timeout, unsigned int RequestCount)
{
	...
	char SendData[32] = "Data by Max Prymakou"; //буфер для передачи 
	int LostPacketsCount = 0; //буфер для передачи 
	int MaxMS = 0;// максимальное время ответа (мс) 
	int MinMS = -1;// миниальное время ответа (мс) 
	...
	IP_OPTION_INFORMATION option = { 100, 0, IP_FLAG_DF, 0, NULL };
		...
		else
		{
			if (pIpe->Status)
			{
				...
				case IP_DEST_PORT_UNREACHABLE:
					cout << "Remote host may be down." << endl; break;
				case IP_REQ_TIMED_OUT:
					cout << "Request timed out." << endl; break;
				case IP_TTL_EXPIRED_TRANSIT:
					cout << "TTL expired in transit." << endl;
					break;
				default:
					cout << "Error code: %ld" << pIpe->Status << endl; break;
				}
			}
		}
	}
	...
	cout << "Статистика Ping " << (int)*(pByte) << "." << (int)*(pByte + 1) << "." << (int)*(pByte + 2) << "." << (int)*(pByte + 3) << endl; cout << "\tПакетов: отправлено = " << RequestCount << ", получено = " << RequestCount - LostPacketsCount << ", потеряно = " << LostPacketsCount << "<" << (int)(100 / (float)RequestCount)* LostPacketsCount << " % потерь>, " << endl;
